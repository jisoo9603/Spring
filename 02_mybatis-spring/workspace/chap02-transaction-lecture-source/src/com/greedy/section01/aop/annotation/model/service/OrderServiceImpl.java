...
public class OrderServiceImpl implements OrderService {
	 ...
	 * REPEATABLE_READ : 트렌젝션이 어떤 필드를 여러 번 읽어도 동일한 값을 읽도록 보장한다.
	 * 					 트랜젝션이 지속되는 동안에는 다른 트랜젝션이 해당 필드를 변경할 수 없다.
	 *    	 	 	 	 오염된 값 읽기, 재현 불가능한 읽기 문제는 해결되지만 허상읽기 문제는 여전히 남아있다.
	 * SERIALIZABLE : 트랜젝션이 테이블을 여러 번 읽어도 정확히 동일한 로우를 읽도록 보장한다.
	 ...
	 * 					결국 다른 트랜젝션은 이 트랜젝션이 커밋 혹은 롤백 되고 수정잠금이 풀릴 떄 까지 기다렸다가
	 * 					읽을 수 밖에 없게 된다.
